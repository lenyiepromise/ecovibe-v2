...
export default function MissionCard({ mission }: { mission: MissionProps }) {
  const progress = Math.min((mission.joined / mission.totalSpots) * 100, 100);
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="mb-4">
          ...
          <div className="flex items-center text-text-muted text-xs gap-1.5">
            <MapPin size={12} />
            <span>{mission.distance} • {mission.location}</span>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-xs text-text-muted mb-2">
            <span className="flex items-center gap-1"><Users size={12}/> {mission.joined}/{mission.totalSpots} Joined</span>
        ...
        <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
          ...
          <button className="bg-white/5 hover:bg-brand-green hover:text-black border border-white/10 hover:border-brand-green text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-all">
            View Details
          </button>
        </div>
